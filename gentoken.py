...
def generateIs(dObj):
    return dObj.getobjtype() + '$' + \
        np.random.choice(['l', 'r'], p=[1. - dObj.getprob1(), dObj.getprob1()])

# given two dummy objects, generate a relation statement


def generaterel(dObj1, dObj2):
    dObj1val = np.random.choice([-1, 1],
                                p=[1. - dObj1.getprob1(), dObj1.getprob1()])
    dObj2val = np.random.choice([-1, 1],
                                p=[1. - dObj2.getprob1(), dObj2.getprob1()])
